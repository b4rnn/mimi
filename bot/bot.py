...
os.environ["OPENAI_API_KEY"] = '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'
...
client = OpenAI(api_key ='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')

audio_dir ='/var/lib/asterisk/sounds'
...
def webhook():
    if request.method == 'POST':
        ...
        response.stream_to_file(audio_dir+"/"+tel[:-1]+".wav")
        subprocess.call(['ffmpeg' ,'-i' ,audio_dir+"/"+tel[:-1]+'.wav', '-acodec', 'pcm_s16le','-ac','1','-ar' ,'8000', audio_dir+"/"+tel+'.wav'])
        end = time.time()
        print(end - start)
        print((tel[:-1],tel))
